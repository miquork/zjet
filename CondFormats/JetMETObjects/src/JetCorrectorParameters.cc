      ...
      if (currentSection == fSection)
        {
          ...
          // Empty and comment-only lines are valid in public JEC payloads.
          // Do not construct a zero-length Record and then query its bin edges.
          if (getTokens(line).empty()) continue;
