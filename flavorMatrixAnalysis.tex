...
\begin{frame}{The stable $p_{T,Z}$ fit now spans 15--800 GeV}
\begin{columns}[c,onlytextwidth]
...
600--800& 4 & 9.05 & shown\\
800--1000& 3 & 4.22 & omitted\\
\bottomrule
\end{tabular}
\par\vspace{0.5em}
The TSV output retains every bin. The figure requires rank 4 and condition number below 100; the low-$p_T$ inversion is algebraically stable but statistically broad.
\end{columns}
\end{frame}
...
\begin{enumerate}
\item Keep PNet QvG at 0.3; the full 2024I purity scan confirms the expected trade-off.
...
\item Use the new flavor-cell $\rho$ and jet PF-muon-fraction profiles in the next production; add heavy-decay-neutrino and direct-versus-splitting ancestry controls before interpreting c/b residuals physically.
\item Require full rank and a bounded condition number separately in every reported pT bin.
\item Add JER, parton-shower/PSWeight, UE, and correlated tagging/fraction variations, then require closure with injected flavor-response shifts before the full 2024 sample.
\end{enumerate}
\vfill
\begin{center}
